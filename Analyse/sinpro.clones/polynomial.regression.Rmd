...
plot.polynomial.regression <- function(df) {
  ...
  x_axis <- seq(1, max(df[,3]), length = max(df[,3] + 1000))
  
  plot(df[,3], df[,2], pch = 1, xlab = "histone", ylab = "Number of hidden edges")
  lines(x_axis, predict(fit1, data.frame(x = x_axis)), col = "red")
  lines(x_axis, predict(fit2, data.frame(x = x_axis)), col = "orange")
  lines(x_axis, predict(fit3, data.frame(x = x_axis)), col = "blue")
  lines(x_axis, predict(fit4, data.frame(x = x_axis)), col = "purple")
  lines(x_axis, predict(fit5, data.frame(x = x_axis)), col = "green3")
}
